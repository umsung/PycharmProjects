...
class SendMail:
    ...
    def __init__(self):
        self.config = Config()
        self.sender = self.config.mail_sender
        self.receiver = self.config.mail_receiver.split(';')

        # 配置第三方 SMTP 服务，如qq
        self.smtpserver = self.config.mail_smtpserver
    ...
    def send_email_by_yagmail(self):        
        email_title =  '自动化测试报告' 
        email_content = "此次一共运行接口个数为 {0} 个，通过个数为 {1} 个，失败个数为 {2} 个，通过率为 {3} %，平均时间为 {4} % " \
            .format(self.get_pass_fail()[0], self.get_pass_fail()[1], self.get_pass_fail()[2], self.get_pass_fail()[3],
                    self.get_pass_fail()[4])
        # 附件列表
        ...
        try:
            # 连接服务器
            # 用户名、授权码、服务器地址
            yag_server = yagmail.SMTP(user= self.sender , password= self.password , host= self.smtpserver)
            # 发送邮件
            yag_server.send(self.receiver, email_title, email_content, email_attachments)
        except Exception as e:
            logging.error('===无法发送邮件，请检查==={}'.format(e))
        finally:
            # 关闭连接
