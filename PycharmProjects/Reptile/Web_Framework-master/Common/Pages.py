...
class Pages():
    def __init__(self,kwargs):
        self.test_msg = getYaml(kwargs["path"])
        _init = {"driver": kwargs["driver"], "test_msg":self.test_msg ,"caseName": kwargs["caseName"]}
        self.yaml_data = self.test_msg[1] if kwargs.get('num') == None else self.test_msg[1][kwargs.get('num')]
        ...
        self.testInfo = self.yaml_data["testinfo"]
        self.testCase = self.yaml_data["testcase"]
        self.test_check = self.yaml_data["check"]
        ...
        if self.isOperate:
            for item in self.test_check:
                ...
                if item.get("check", Element.DEFAULT_CHECK) == Element.DEFAULT_CHECK and not resp["result"]:
                    result = False
                    break
                if item.get("check", Element.DEFAULT_CHECK) == Element.COMPARE and self.is_get and  item.get("index","NaN") not in  resp["text"]:  # 历史数据和实际数据对比
                        
                    result = False
                    break
                # 历史数据和实际数据对比
                # if item.get("check", Element.DEFAULT_CHECK) == Element.COMPARE and self.is_get and resp["text"] not in self.get_value:  # 历史数据和实际数据对比
                ...
                if item.get("check", Element.DEFAULT_CHECK) == Element.CONTRARY_GETVAL and self.is_get and resp["text"] \
                        in self.get_value:
                    # m = get_error(
