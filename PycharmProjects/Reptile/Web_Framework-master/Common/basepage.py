...
class BasePage(Base_Web_Page, Base_App_Page):
    ...
    def operate_by(self, operate, testInfo):
        ...
        try:
            info = "定位元素:%s__定位方式:%s__操作方法:%s__具体信息:%s" % (
                operate.get("element_info", " "), operate.get("find_type"), operate.get("operate_type", " "),
                operate.get("msg", " "))

            logging.info(testInfo[0]["id"] + "_" + testInfo[0]["title"] + "_操作步骤：==" + info)  # 记录日志
    ...
    def send_keys(self, operate):
        ...
        """
        time.sleep(0.5)
        self.elements_by(operate).send_keys(operate.get("msg"))
        return {"result": True}
    ...
    def get_windows_img(self, name):
        ...
        if not os.path.exists(Element.IMAGES):
            os.mkdir(Element.IMAGES)
